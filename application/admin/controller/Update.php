...
class Update extends Admin{
	...
	private function checkVersion(){ 
		if(extension_loaded('curl')){
			$url = 'http://www.twothink.cn/index1.html/index/index/check_version';
			$params = array(
	...
	private function update($version){
		...
		if($backupFile){
			...
			$dd=$zip->create('addons,extend,application,thinkphp,.htaccess,install.php,index1.html');
			$this->showMsg('成功完成重要程序备份,备份文件路径:<a href=\''.'/'.$backupallPath.'\'>'.$backupallPath.'</a>, 耗时:'.debug('start1','stop1').'s','success');
		} 
		/* 获取更新包 */
		//获取更新包地址
		$updatedUrl = 'http://www.twothink.cn/index1.html/index';
