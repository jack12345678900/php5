...
<div class="data-table table-striped">
    <table>
        ...
        <tbody>
        <?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zushou): $mod = ($i % 2 );++$i;?>
        <tr>
            <td><input class="ids row-selected" type="checkbox" name="" id="" value="<?php echo $activity['id']; ?>"> </td>
            <td><?php echo $zushou['id']; ?></td>
            <td><?php echo $zushou['name']; ?></td>
            <td><?php echo $zushou['tel']; ?></td>
            <td><?php echo $zushou['title']; ?></a></td>
            <td><?php echo !empty($zushou['form']) && $zushou['form']==1?"出租":"出售"; ?></a></td>
            <td><?php echo $zushou['price1']; ?><?php echo $zushou['price2']; ?></a></td>
            <td><?php echo date("Y-m-d H:i:s",$zushou['create_time']); ?></td>
            <td><?php echo $zushou['update_time']; ?></td>
            <td><?php echo !empty($zushou['status']) && $zushou['status']==1?"已处理":"待处理"; ?>
                <a href="<?php echo url('update?method=update&id='.$zushou['id']); ?>" id="tongguo">处理</a>
                </td>
            <td>
                <a title="编辑" href="<?php echo url('edit?id='.$zushou['id'].'&pid='.$pid); ?>">编辑</a>
                <a class="confirm ajax-get" title="删除" href="<?php echo url('del?id='.$zushou['id']); ?>">删除</a>
            </td>
        </tr>
